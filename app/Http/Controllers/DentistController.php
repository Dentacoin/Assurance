...
class DentistController extends Controller
{
    ...
    protected function notifyPatientForSuccessfulWithdraw(Request $request) {
        ...
        if (!empty($contract)) {
            $this->successfulBlockchainWithdraw($contract, $request->input('transaction_hash'));
        ...
        if(!empty($contract)) {
            ...
            if(!empty($records)) {
                ...
                if($checkUp != NULL && $teethCleaning != NULL) {
                    ...
                    return response()->json(['success' => true, 'html' => $popupHtml, 'data' => array($checkUp->id => $checkUp), 'currentCheckUpCount' => $currentCheckUpCount, 'check_ups_per_year' => $contract->check_ups_per_year]);
                    ...
                    return response()->json(['success' => true, 'html' => $popupHtml, 'data' => array($teethCleaning->id => $teethCleaning), 'currentTeethCleaningsCount' => $currentTeethCleaningsCount, 'teeth_cleaning_per_year' => $contract->teeth_cleaning_per_year]);
                }
            } else {
