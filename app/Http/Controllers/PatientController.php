...
class PatientController extends Controller {
    ...
    public function getCheckUpOrTeethCleaning($type, $slug, $from, $to) {
        var_dump($from);
        var_dump($to);
        $checkUps = DB::connection('mysql')->table('contract_checkups')->leftJoin('temporally_contracts', 'contract_checkups.contract_id', '=', 'temporally_contracts.id')->select('contract_checkups.*')->where(array('temporally_contracts.patient_id' => session('logged_user')['id'], 'temporally_contracts.status' => 'active', 'temporally_contracts.slug' => $slug, 'contract_checkups.type' => $type, 'contract_checkups.approved_by_dentist' => true))->whereBetween('contract_checkups.date_at', array($from, $to))->get()->all();
