        ...
        if($encrypted_html_by_patient && !isset($encrypted_html_by_patient->error) && $encrypted_html_by_dentist && !isset($encrypted_html_by_dentist->error)) {
            ...
            if($ipfs_hash->response_obj && $ipfs_hash->response_obj->success) {
                ...
                if(sizeof($this_patient_having_contracts) == 1) {
                    ...
                    $send_eth_amount = (new \App\Http\Controllers\APIRequestsController())->sendETHamount(htmlentities($this->node_encrypt($contract->patient_address, getenv('NODEJS_ADDITIONAL_API_METHOD'), getenv('NODEJS_ADDITIONAL_API_KEY'))));
